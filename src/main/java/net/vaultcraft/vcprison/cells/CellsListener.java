    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if(possibleCell == null) {
            if(event.getClickedBlock().getType() == Material.IRON_DOOR_BLOCK && event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
                ...
                Door door1;
                if(door.isTopHalf())
                    door1 = (Door) event.getClickedBlock().getRelative(BlockFace.DOWN).getState().getData();
                else
                    door1 = (Door) event.getClickedBlock().getRelative(BlockFace.UP).getState().getData();
                if(!door.isOpen()) {
                    door.setOpen(true);
                    door1.setOpen(true);
